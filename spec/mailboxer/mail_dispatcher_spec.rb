...
describe Mailboxer::MailDispatcher do
  ...
  describe 'call' do
    context 'no emails' do
      ...
      its(:call) { is_expected.to be false }
    end
  ...
  describe 'send_email' do
    ...
    context 'without custom_deliver_proc' do
      ...
      it 'triggers standard deliver chain' do
        ...
        expect(receipt1).to receive(:assign_attributes).with({ delivery_method: :email,
                                                               message_id: '123@local.com' }).and_return email
  ...
  describe 'mailer' do
    ...
    context 'mailable is a Message' do
      ...
      its(:mailer) { is_expected.to be Mailboxer::MessageMailer }
      ...
      context 'mailer class is selected by searching for the constant' do
        ...
        let(:mailable) { +'I am a string' }

        its(:mailer) { is_expected.to eq Object.const_get(:StringMailer) }
      end
    end
  end
end
