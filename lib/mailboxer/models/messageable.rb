    ...
    module Messageable
      ...
      included do
        ...
        has_many :receipts, lambda {
                            ...
                            }, class_name: 'Mailboxer::Receipt', dependent: :destroy, as: :receiver
      end
      ...
      unless defined?(Mailboxer.name_method)
        ...
        define_method Mailboxer.name_method do
          ...
          "You should add method :#{Mailboxer.name_method} in your Messageable model"
        end
      end
      ...
      unless defined?(Mailboxer.email_method)
        ...
        define_method Mailboxer.email_method do |_object|
          super
        rescue NameError
          "You should add method :#{Mailboxer.email_method} in your Messageable model"
        end
      end
      ...
      def send_message(recipients, msg_body, subject, sanitize_text = true, attachments = nil,
                       message_timestamp = Time.zone.now, with_email: true)
      ...
      def mark_as_deleted(obj)
        case obj
        when Mailboxer::Receipt
          obj.mark_as_deleted if obj.receiver == self
        when Mailboxer::Message, Mailboxer::Notification
      ...
      def search_messages(query)
        if Mailboxer.search_engine == :pg_search
          ...
          @search.results.map(&:conversation).uniq
        end
      end
    end
  end
