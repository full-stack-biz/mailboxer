...
module Mailboxer
  class Mailbox
    ...
    def notifications(options = {})
      ...
      notifs = Mailboxer::Notification.recipient(messageable).where(type: nil).order(created_at: :desc,
                                                                                     id: :desc)
      notifs = notifs.unread if options[:read] == false || options[:unread]
    ...
    def conversations(options = {})
      ...
      conv = conv.unread(messageable) if options[:read] == false || options[:unread]
