...
module Mailboxer
  class Notification < ApplicationRecord
    ...
    scope :expired, -> { where("#{Mailboxer::Notification.quoted_table_name}.expires < ?", Time.zone.now) }
    scope :unexpired, lambda {
      where("#{Mailboxer::Notification.quoted_table_name}.expires is NULL OR #{Mailboxer::Notification.quoted_table_name}.expires > ?", Time.zone.now)
    }
    ...
    def expired?
      expires.present? && (expires < Time.zone.now)
    end
    ...
    def recipients
      return Array.wrap(@recipients) if @recipients.present?
